...
df=as.data.frame(cbind(file_list_UG,unlist(EPA_Random_Ht_Single),shift_list_estimated))
colnames(df)=c('graph','EPA','shift_list_estimated')


# Save results for Beta=3
# write.csv(df,"./Covid 19 Results EPA (Beta=3).csv")

# Save results for Beta=4
write.csv(df,"./Covid 19 Results EPA (Beta=4).csv")
